...
const categories = ["Rent", "Groceries", "Electricity", "Gas", "Internet", "Cleaning", "Household", "Maintenance", "Transport", "Other"];
const categoryIcons:Record<string,string> = { Rent:"⌂", Groceries:"◒", Electricity:"ϟ", Gas:"◉", Internet:"⌁", Cleaning:"✦", Household:"□", Maintenance:"◇", Transport:"→", Other:"•" };
...
export default function FairNestApp({ user }:{ user:{name:string;email:string} }) {
  ...
  useEffect(() => {
    ...
    void Promise.resolve(context.registerTool({
      name:"add_shared_expense", title:"Add shared expense", description:"Add an equally split expense to the currently selected FairNest household.",
  ...
  return (
    <main className="app-shell">
      <aside className="sidebar">
        <div className="brand"><div className="brand-mark small">F</div><span>FairNest</span></div>
...
function RecurringView({active,onAdd,onPost}:{active:ActiveData;onAdd:()=>void;onPost:(id:string)=>void}) { return <section className="page-card"><div className="section-heading recurring-head"><div><h2>Recurring bills</h2><p>Keep rent and regular utilities ready to post.</p></div><Button onClick={onAdd}><Plus/>New recurring bill</Button></div><div className="recurring-grid">{active.recurring.map((r)=><article key={r.id}><div className="recurring-icon"><CalendarClock/></div><div className="recurring-copy"><span>{r.category}</span><h3>{r.description}</h3><p>Paid by {r.payer_name} · {r.cadence}</p></div><strong>{formatMoney(r.amount_cents,active.household.currency)}</strong><div className="due-line"><span>Next due {displayDate(r.next_due_date)}</span><Button variant="outline" size="sm" onClick={()=>onPost(r.id)}>Post now</Button></div></article>)}{!active.recurring.length&&<EmptyState icon={<Repeat2/>} title="No recurring bills" copy="Save rent, internet, or subscriptions so they are ready each month." action={onAdd}/>}</div></section>; }
