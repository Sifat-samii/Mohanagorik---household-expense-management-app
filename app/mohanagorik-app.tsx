...
type ActiveData = {
  ...
  balances:Balance[]; suggestedSettlements:Suggestion[]; selectedMonth:string; monthlyExpenses:Expense[]; monthTotalCents:number; categories:Array<{name:string;amountCents:number}>; memberSpending:Array<{memberId:number;name:string;amountCents:number}>;
};
...
function Overview({active,balance,onExpense,onSettle}:{active:ActiveData;balance:number;onExpense:()=>void;onSettle:(s:Suggestion)=>void}) {
  ...
  const maxMemberSpending = Math.max(...active.memberSpending.map((member)=>member.amountCents),1);
  ...
  <div className="summary-card member-spending-card"><div className="section-heading"><div><h2>Paid by member</h2><p>Total each person paid in {monthLabel}</p></div></div><div className="category-bars member-bars">{active.memberSpending.map((member)=><div key={member.memberId}><div><span>{member.name}</span><b>{formatMoney(member.amountCents,active.household.currency)}</b></div><i><em style={{width:`${member.amountCents ? Math.max(5,member.amountCents/maxMemberSpending*100) : 0}%`}}/></i></div>)}</div></div>
