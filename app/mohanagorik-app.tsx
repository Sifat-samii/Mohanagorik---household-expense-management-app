...
type Member = { id:number; display_name:string; avatar_choice:string; avatar_key:string|null };
...
type MutationResult = { ok?:boolean; householdId?:string; expenseId?:string; error?:string };
    ...
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <div className="sidebar-footer">
          ...
          <button className="icon-link" aria-label="Sign out" onClick={()=>void signOut()}><LogOut size={17}/></button>
        </div>
      </aside>
...
function RecurringView({active,onAdd,onPost}:{active:ActiveData;onAdd:()=>void;onPost:(id:string)=>Promise<unknown>}) {
  ...
  const secondsLeft=(item:Recurring)=>now === null ? 0 : Math.max(0,Math.ceil((Date.parse(item.last_posted_at||"") + 15_000 - now)/1000)||0);
